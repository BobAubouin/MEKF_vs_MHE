...
def metrics_function(path: str, patient_id: int, stop_time: int, pred_time: int = -1, plot: bool = False):
    ...
    d = parameters.loc[parameters['Time'] == stop_time, ['d']].to_numpy()[0]
...
if __name__ == '__main__':
    if bool_data[0]:
        try:
            metrics_MEKF_P = pd.read_csv('data_extended/metrics_MEKF.csv', index_col=0)
        except FileNotFoundError:
            with mp.Pool(mp.cpu_count()) as pool:
                res = list(tqdm(pool.imap(partial(one_line, path=mekf_path, stop_time_list=stop_time_list,
                                                  pred_time=pred_time), range(number_of_patients)), total=number_of_patients, desc='MEKF_P'))
            metrics_MEKF_P = pd.concat(res)
            metrics_MEKF_P.to_csv('data_extended/metrics_MEKF.csv')
    if bool_data[1]:
        try:
            metrics_MHE = pd.read_csv('data_extended/metrics_MHE.csv', index_col=0)
        except FileNotFoundError:
            with mp.Pool(mp.cpu_count()) as pool:
            ...
            metrics_MHE.to_csv('data_extended/metrics_MHE.csv')

    if bool_data[2]:
        try:
            metrics_MM = pd.read_csv('data_extended/metrics_MM.csv', index_col=0)
        except FileNotFoundError:
            with mp.Pool(mp.cpu_count()) as pool:
                res = list(tqdm(pool.imap(partial(one_line, path=mm_path, stop_time_list=stop_time_list,
                                                  pred_time=pred_time), range(number_of_patients)), total=number_of_patients, desc='MM'))
            metrics_MM = pd.concat(res)
            metrics_MM.to_csv('data_extended/metrics_MM.csv')
    ...
    if bool_data[2]:
        mean_MM = metrics_MM.mean()
        std_MM = metrics_MM.std()
    ...
    if bool_data[2]:
        plt.fill_between(stop_time_list, mean_MM-std_MM, mean_MM+std_MM,
    ...
    if bool_data[2]:
        plt.plot(stop_time_list, mean_MM, label='MM', color=mcolors.TABLEAU_COLORS['tab:green'])
    ...
    plt.xlabel('stop time (min)')
    plt.grid()
    savepath = "figures/stats_comp_extended.pdf"
    ...
    plt.figure()
    if bool_data[0]:
        plt.plot(stop_time_list, metrics_MEKF_P.max(), label='MEKF', color=mcolors.TABLEAU_COLORS['tab:orange'])
    if bool_data[1]:
        plt.plot(stop_time_list, metrics_MHE.max(), label='MHE', color=mcolors.TABLEAU_COLORS['tab:blue'])
    if bool_data[2]:
        plt.plot(stop_time_list, metrics_MM.max(), label='MM', color=mcolors.TABLEAU_COLORS['tab:green'])

    plt.legend()
    plt.ylabel('metrics')
    plt.xlabel('stop time (min)')
    plt.grid()
    savepath = "figures/stats_max_extended.pdf"
    plt.savefig(savepath, bbox_inches='tight', format='pdf')
    plt.show()
