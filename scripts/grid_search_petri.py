...
# %% Grid search
results = pd.DataFrame(columns=['R', 'lambda_2', 'nu', 'epsilon', 'objective_function'])

for R, lambda_2, nu, epsilon in tqdm(product(R_list, lambda_2_list, nu_list, epsilon_list), desc='Grid search', total=len(R_list)*len(lambda_2_list)*len(nu_list)*len(epsilon_list)):
