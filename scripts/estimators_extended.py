...
class EKF_integrator_new:
    ...
    def __init__(self, A: list, B: list, BIS_param: list, ts: float, x0: list = np.ones((9, 1))*1.e-3,
        ...
        self.Ad = np.block([[self.Ad, np.zeros((8, 1))], [np.zeros((1, 8)), 1]])
...
class MEKF_MHE:
    ...
    B : list
        Input matric of the continuous system dx/dt = Ax + Bu.
    mekf_param : list
        list of MEKF parameter [R, Q, P0, eta0, grid_vector, lambda_1, lambda_2, nu, epsilon]
    mhe_param : list
        list of MHE parameter [R, Q, theta, N_MHE]
    ts : float, optional
    ...
    def __init__(self, A: list, B: list, BIS_param: list, mekf_param: list, mhe_param: list, ts: float = 1, switch_time: float = 120) -> None:
        ...
        self.time = 0
        self.X = np.zeros((12, mhe_param[3]))
        self.bis = np.zeros(mhe_param[3])
        self.u = np.zeros((2*mhe_param[3]))
